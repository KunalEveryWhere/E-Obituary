...
export default function Createnew() {
  ...
  const [deleteOn, setDeleteOn] = useState(`${(new Date().getFullYear())}-${(new Date().getMonth() + 2)}-${(new Date().getDate())}T00:00`);
  ...
  const [uuidFromDB, setUuidFromDB] = useState([]);
  const [uniqueCode, setUniqueCode] = useState('');

  const [tempo, setTempo] = useState('');
  const [viewURLIsValid, setViewURLIsValid] = useState('false');
  ...
  const handleSubmitToDB = async (e) => {
    ...
    //Setting required DB variables
    // setSiteFormat("template"+selectedFormat);
      ...
      <div className='row flex-center flex'>
        <div className="col-10 form-widget" aria-live="polite">
            <div className='card'>
              ...
              <label htmlFor="addFile">請上傳往生者照片 *（建議照片比例為直立長方形或正方形，且人物在中間）</label>
              {preview ? <div className='' style={{marginTop: '1em', padding: '1em', border: '1px solid grey', borderRadius: '6px'}}>
                <img src={preview} style={{height: '60px'}} />
                </div> : ''}
                <input
                  type="file"
                  accept="image/*"
                ...
                <button className='renderList-button renderList-url' style={{marginTop: '2.2em'}} disabled={!uuid} onClick={(e) => {e.preventDefault(); window.open(customerSiteDomain+uuid, '_blank');}}><img src={URLIcon} alt="view-icon"/></button>
                          ...
                          <ReactQuill theme="snow" value={parameter1} onChange={setParameter1} readOnly={false} className='richEditor' />

                          <label htmlFor="parameter2" style={{marginTop: '2em'}}>孝眷名單</label>
                          <ReactQuill theme="snow" value={parameter2} onChange={setParameter2} readOnly={false} className='richEditor' />

                          <label htmlFor="parameter3" style={{marginTop: '2em'}}>親眷名單</label>
                          <ReactQuill theme="snow" value={parameter3} onChange={setParameter3} readOnly={false} className='richEditor' />

                          <label htmlFor="parameter4" style={{marginTop: '2em'}}>儀式地點</label>
                          <ReactQuill theme="snow" value={parameter4} onChange={setParameter4} readOnly={false} className='richEditor' />

                          <label htmlFor="parameter5" style={{marginTop: '2em'}}>禮俗提醒</label>
                          <ReactQuill theme="snow" value={parameter5} onChange={setParameter5} readOnly={false} className='richEditor' />
...
const RadioButton = ({ label, value, onChange, disabled }) => {
  return (
    <label>
      ...
      <img src={label} alt='format-image' />
    </label>
  );
};
